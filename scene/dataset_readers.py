...
def convert_from_scene_camera(viewpoint_cam):
    intrinsic_matrix, extrinsic_matrix = viewpoint_cam.get_calib_matrix_nerf()
    from model import libcore
    cam = libcore.Camera()
    cam.w = viewpoint_cam.image_width
    cam.h = viewpoint_cam.image_height
    cam.fx = intrinsic_matrix[0, 0].item()
    cam.fy = intrinsic_matrix[1, 1].item()
    cam.cx = intrinsic_matrix[0, 2].item()
    cam.cy = intrinsic_matrix[1, 2].item()
    cam.R = extrinsic_matrix[:3, :3].detach().cpu().numpy()
    cam.setTranslation(extrinsic_matrix[:3, 3].detach().cpu().numpy())
    return cam
