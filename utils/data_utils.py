...
def retrieve_norms_barycentric(normals, faces, fidxs, barys):
    triangle_norms = normals[faces].float()

    if len(triangle_norms.shape) == 3:
        sample_norms = torch.einsum('nij,ni->nj', triangle_norms[fidxs], barys)
    elif len(triangle_norms.shape) == 4:
        sample_norms = torch.einsum('bnij,ni->bnj', triangle_norms[:, fidxs, ...], barys)
    else:
        raise NotImplementedError
    
    return thf.normalize(sample_norms, dim=-1)

def construct_quaternion_barycentric(vertices, normals, faces, fidxs, barys):
    vec_x = thf.normalize(vertices[faces[fidxs][:, 1]] - vertices[faces[fidxs][:, 0]], dim=-1)
    vec_z = retrieve_norms_barycentric(normals, faces, fidxs, barys)

    vec_y = thf.normalize((vec_z.cross(vec_x, dim=-1)), dim=-1)
    vec_x = vec_y.cross(vec_z)
    rotations = torch.stack([vec_x, vec_y, vec_z], dim=1)
    quats = py3d_trans.matrix_to_quaternion(rotations)
    return quats

def construct_quaternion_triangle(vertices, normals, faces):
    vec_x = thf.normalize(vertices[faces[:, 1]] - vertices[faces[:, 0]], dim=-1)
    vec_z = retrieve_norms_barycentric(normals, faces, torch.arange(faces.shape[0]).to(faces), 
                                       torch.full((faces.shape[0], 3), 1.0 / 3).to(normals))

    vec_y = thf.normalize((vec_z.cross(vec_x, dim=-1)), dim=-1)
    vec_x = vec_y.cross(vec_z, dim=-1)
    rotations = torch.stack([vec_x, vec_y, vec_z], dim=1)
    quats = py3d_trans.matrix_to_quaternion(rotations)
    return quats
