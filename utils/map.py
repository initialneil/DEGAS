...
# area weighted face attributes to vertex
def face_attr_to_vertex(verts, faces, face_attrs):
    face_areas = calc_face_areas(verts, faces)

    verts_attrs = torch.zeros(verts.shape[0], face_attrs.shape[-1]).to(face_attrs)

    # NOTE: this is already applying the area weighting as the magnitude
    # of the cross product is 2 x area of the triangle.
    verts_attrs = verts_attrs.index_add(
        0, faces[:, 0], face_areas * face_attrs
    )
    verts_attrs = verts_attrs.index_add(
        0, faces[:, 1], face_areas * face_attrs
    )
    verts_attrs = verts_attrs.index_add(
        0, faces[:, 2], face_areas * face_attrs
    )
