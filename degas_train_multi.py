...
def get_loader(dataloader_list):
    dataloader = np.random.choice(dataloader_list)
    print(f'[get_loader] using loader num_frames = {len(dataloader.dataset.frm_list)}')
    return dataloader
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dat_dirs', type=str, required=True, nargs='+')
    ...
    config = load_from_config(ckpt_info['configs'], cli_args=extras)
    ...
    frameset_train_list = []
    dataloader_list = []
    for dat_dir in args.dat_dirs:
        config.dataset.dat_dir = dat_dir
        frameset_train = make_frameset_data(config.dataset, split='train')
        frameset_train_list.append(frameset_train)
    ...
    if ckpt_info['ckpt_fn'] is not None:
        ...
        gs_model.pca = frameset_train_list[0].pca

    gs_model.update_to_pose(cano_params)

    gs_optim = DEGASOptimizer(gs_model, smplx_optim=None, optimizer_config=config.optim)
    ...
    dataloader = get_loader(dataloader_list)
    ...
    while network_gui.conn is not None:
        network_gui.render_to_network(gs_model, pipe, work_dir)
