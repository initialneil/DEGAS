...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # backup hands mean for later use
        self.register_buffer('hands_meanl',
                             to_tensor(data_struct.hands_meanl, dtype=self.dtype))
        self.register_buffer('hands_meanr',
                             to_tensor(data_struct.hands_meanr, dtype=self.dtype))
...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        # optional use v_template
        v_template = kwargs['v_template'] if 'v_template' in kwargs else self.v_template

        # skinning
        ret = lbs(shape_components, full_pose, v_template,
...
class FLAME(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if os.path.exists(landmark_bcoord_filename.replace('.pkl', '.pt')):
            landmarks_data = torch.load(landmark_bcoord_filename.replace('.pkl', '.pt'))
        else:
            try:
                ...
                with open(landmark_bcoord_filename, 'rb') as fp:
                    cc = fp.read()
                with open(landmark_bcoord_filename.replace('.pkl', '.rewrite.pkl'), 'wb') as fp:
                    for line in cc.splitlines():
                        fp.write(line + str.encode('\n'))
                with open(landmark_bcoord_filename.replace('.pkl', '.rewrite.pkl'), 'rb') as fp:
                    landmarks_data = pickle.load(fp, encoding='latin1')
                torch.save(landmarks_data, landmark_bcoord_filename.replace('.pkl', '.pt'))
