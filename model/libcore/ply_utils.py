...
def saveVNmapToPly(fn, vmap, nmap, color=None, tag='saveVNmapToPly'):
    try:
        import torch
        if isinstance(vmap, torch.Tensor):
            vmap = vmap.detach().cpu()
        if isinstance(nmap, torch.Tensor):
            nmap = nmap.detach().cpu()
    except:
        pass
...
def savePairedPointsToPly(fn, pts0, pts1, nml0=None, nml1=None):
    if pts0 is None or pts1 is None or pts0.shape[0] == 0 or pts0.shape[0] != pts1.shape[0]:
        print(f'[savePairedPointsToPly] shape mismatch')
        return
    
    try:
        import torch
        if isinstance(pts0, torch.Tensor):
            pts0 = pts0.detach().cpu()
...
##################################################
def save_openpose_keypoints_to_ply(fn, verts, colors=None):
    if len(verts.shape) == 3:
        verts = verts[0]
    ...
    link_list = [
        ...
        [81, 82, 83, 84, 85],
        # mouth outer (loop)
        [98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 98],
        # mouth inner (loop)
        [110, 111, 112, 113, 114, 115, 116, 117, 110],
        # left eyebrow 
        [72, 73, 74, 75, 76],
        # right hand
        [46, 47, 48, 49, 50],
    ...
    for link in link_list:
        for i in range(len(link) - 1):
            ply_writer.addEdgeByIdx(link[i], link[i + 1])
