...
# ======================================================= #
OmegaConf.register_new_resolver('int', lambda i: int(i))
OmegaConf.register_new_resolver('if_else', lambda cond, a, b: a if cond else b)
OmegaConf.register_new_resolver('int2str', lambda x, fmt: f'{x:{fmt}}')
OmegaConf.register_new_resolver('not', lambda a: not a)
# ======================================================= #
...
def load_from_config(yaml_files, dat_dir=None, cli_args=[]):
    if dat_dir is not None:
        for i in range(len(yaml_files)):
            ...
            yaml_files[i] = yaml_files[i].replace('{dat_dir}', dat_dir)
