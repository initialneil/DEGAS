...
class MeshCpu:
    ...
    def __init__(self, mesh_fn=None, atlas_fn=None, use_original_N=False):
        ...
        if atlas_fn is not None:
            import cv2
            self.atlas = cv2.imread(atlas_fn, cv2.IMREAD_UNCHANGED)
    ...
    def read_obj(self, obj_fn):
        ...
        # TC
        if self.TC is not None and len(self.TC.shape) == 1:
            self.TC = self.TC[None, :]
    ...
    def save_to_obj(self, obj_fn, mtl_fn=None, atlas_fn=None):
        os.makedirs(Path(obj_fn).parent, exist_ok=True)
    ...
    def save_to_ply(self, ply_fn):
        ...
        with_color = self.VC is not None
        if with_color:
            ply_writer.with_color = True
            ply_writer.colors = self.VC
...
def save_mesh_to_obj(fn, verts, faces):
    if _with_torch:
...
def save_mesh_to_ply(fn, verts, faces):
    if _with_torch:
        if isinstance(verts, torch.Tensor):
...
##################################################
# select faces by used vertex indices
# mode 'all': keep the face if all vertices in the face is used
# mode 'any': keep the face if any vertices in the face is used
def select_faces_by_vertex_index(F, used_vidxs, mode='all', with_selected_idxs=False):
    used_marks = np.in1d(F, used_vidxs).reshape(F.shape)
    if mode == 'all':
        select_idxs = used_marks.all(axis=-1)
    else:
        select_idxs = used_marks.any(axis=-1)

    if with_selected_idxs:
        return F[select_idxs], select_idxs
    else:
        return F[select_idxs]
