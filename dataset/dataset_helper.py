...
def make_frameset_data(dataset_config, split='train', frm_list=None):
    ...
    frameset_type = dataset_config.get('frameset_type', 'color_frames')
    if frameset_type == 'frameset' or frameset_type == 'color_frames':
        from .frameset_data import FramesetDataset
        frameset = FramesetDataset(dataset_config, split=split, frm_list=frm_list)
    elif frameset_type == 'actorshq':
        from .actorshq_data import ActorsHQDataset
        frameset = ActorsHQDataset(dataset_config, split=split, frm_list=frm_list)
    elif frameset_type == 'imavatar':
        from .imavatar_data import IMavatarDataset
        frameset = IMavatarDataset(dataset_config, split=split, frm_list=frm_list)
    elif frameset_type == 'dreams' or frameset_type == 'dreams-avatar':
        # DREAMS-AVATAR native bundle: cameras.json + smplx.npz + videos/camNN.mp4.
        # Yields the same scene_cameras/smplx_params a batch as the actorshq path.
        from .dreams_data import DREAMSDataset
        frameset = DREAMSDataset(dataset_config, split=split, frm_list=frm_list)
    else:
        raise NotImplementedError(f'unknown frameset_type: {frameset_type!r} '
                                  f'(color_frames | actorshq | imavatar | dreams)')
